...
function Task(props) {
  ...
  const [discripton, setDiscripton] = useState(props.discripton);
  ...
  return (
    <div className={`border m-2 rounded-xl shadow `}>
      <div className={`p-2  border-b flex rounded-t-xl ${color}`}>
        ...
        <div
          className={`${color} w-9 h-7 focus:outline-none rounded ml-2 relative border shadow`}
          onClick={openEdit}
        >
          {isEditMode ? (
            <div
              className={`bg-white border rounded p-1 absolute right-0 flex shadow-2xl`}
            >
              <div
                onClick={() => setColor("bg-red-500")}
                className="bg-red-500 w-7 h-7 focus:outline-none rounded border m-2 shadow"
              ></div>
              <div
                onClick={() => setColor("bg-green-500")}
                className="bg-green-500 w-7 h-7 focus:outline-none rounded border m-2 shadow"
              ></div>
              <div
                onClick={() => setColor("bg-blue-500")}
                className="bg-blue-500 w-7 h-7 focus:outline-none rounded border m-2 shadow"
              ></div>
              <div
                onClick={() => setColor("bg-yellow-500")}
                className="bg-yellow-500 w-7 h-7 focus:outline-none rounded border m-2 shadow"
              ></div>
              <div
                onClick={() => setColor("bg-white")}
                className="bg-white w-7 h-7 focus:outline-none rounded border m-2 shadow"
              ></div>
            </div>
          ) : (
            <></>
